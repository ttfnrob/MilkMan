...
zooid = "AMW0000uxp"
s = Subject.collection.find_one({"zooniverse_id": zooid})
width = float(s.metadata.size.split("x")[0])
height = float(s.metadata.size.split("x")[1])
pxscale = width/800.0
clat = float(s.coords[0])
clon = float(s.coords[1])
image_url = s.location.standard

print ""+str(width)+" "+str(height)+" "+str(pxscale)

### Import data from CSV
###
lon = [ clon-width , clon+width  ]
lat = [ clat-height, clat+height ]
path_to_csv = "/Users/Rob/Projects/MilkMan/data/raw/annotations/bubble_raw_20140123090449.csv"
...
xoff = width/2.0
yoff = height/2.0
ax.set_xlim( clon+xoff , clon-xoff )
ax.set_ylim( clat-yoff , clat+yoff )
...
pl.savefig(directory+zooid+"_reduction_"+str(l)+"_"+str(s)+"_"+str(r)+"_"+stamp+".png", dpi=300, bbox_inches='tight')
pl.close()
